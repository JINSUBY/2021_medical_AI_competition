...
for d in random.sample(DATA_dataset_dicts, 20):
    ...
    txt = open(os.path.join(cfg.OUTPUT_DIR, d["file_name"].split("/")[-1].replace(".jpg", ".txt")), "w")
    data = f.readlines()
    for i in data:
        i = i.split("\t")
        x = int(i[1])
        ...
        if (x == 0):
            ...
            if i[0] == "8":
                #im[i[1], i[2]] = [0, 255, 0]
                #im = cv2.line(im, (x, y), (x, y), red_color, 5)
                count = 0
                while(count<2):
                    x -= 1
                    if(list(im[y,x]) != [255, 255, 255]):
                        print(count)
                        count += 1
                im = cv2.line(im, (x, y), (x, y), blue_color, 5)
                txt.write(i[0] + "\t" + str(x) + "\t" + str(y) + "\r\n")
            else: #일단은 이렇게 해놓는다..
                print("pass!!")
                txt.write(i[0] + "\t" + "0" + "\t" + "0" + "\r\n")
    txt.close()
